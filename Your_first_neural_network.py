...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            output_error_term = error * 1 #dreivative of x is 1
            
            # TODO: Calculate the hidden layer's contribution to the error
            hidden_error = np.dot(self.weights_hidden_to_output, output_error_term)
            #hidden_error = np.dot(hidden_outputs, self.weights_hidden_to_output)
            #hidden_error_term = (output_error_term * self.weights_hidden_to_output) * hidden_outputs * (1-hidden_outputs)
...
class TestMethods(unittest.TestCase):
    ...
    def test_train(self):
        ...
        print("network.weights_hidden_to_output {}".format(network.weights_hidden_to_output))
        self.assertTrue(np.allclose(network.weights_hidden_to_output, 
                                    np.array([[ 0.37275328], 
                                              [-0.03172939]])))
        print("network.weights_input_to_hidden {}".format(network.weights_input_to_hidden))
